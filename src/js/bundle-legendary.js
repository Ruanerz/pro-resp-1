...
async function createIngredientTree1(itemData, parent = null) {
  ...
  if (itemData.components && Array.isArray(itemData.components)) {
    for (const componentData of itemData.components) {
      const child = await createIngredientTree1(componentData, ingredient);
      if (child) {
...
async function createIngredientTree3(itemData, parent = null) {
  ...
  const shouldSkipMarketCheck = (id, name) => {
    ...
    if (name) {
      ...
      if (lowerName.includes('piedra rúnica') && id !== 96722) return true;
  ...
  if (itemData.components && Array.isArray(itemData.components)) {
    for (const componentData of itemData.components) {
      const child = await createIngredientTree3(componentData, ingredient);
      if (child) {
        ingredient.addComponent(child);
      }
    }
